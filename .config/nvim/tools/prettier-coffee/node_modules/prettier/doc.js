(function (global, factory) {
  ...
  function fits(next, restCommands, width, options, mustBeFlat) {
    ...
    while (width >= 0) {
      ...
      const [ind, mode, doc, visibleGroups] = cmds.pop();
      ...
      if (typeof doc === "string") {
        ...
        switch (doc.type) {
          ...
          case "indent":
            cmds.push([makeIndent(ind, options), mode, doc.contents, visibleGroups]);
            break;

          case "align":
            cmds.push([makeAlign(ind, doc.n, options), mode, doc.contents, visibleGroups]);
            break;
          ...
          case "group":
            {
              ...
              if (!docBreaks && doc.breakIfVisibleTypeBroke) {
                const {
                  offset,
                  type
                } = doc.breakIfVisibleTypeBroke;

                if (getVisibleGroupMode({
                  type,
                  offset,
                  visibleGroups
                }) === MODE_BREAK) {
              ...
              cmds.push([ind, docBreaks ? MODE_BREAK : mode, doc.contents, doc.visibleType ? [...(visibleGroups || []), {
                type: doc.visibleType,
                mode: docBreaks ? MODE_BREAK : mode
              }] : visibleGroups]);
          ...
          case "if-break":
            {
              const groupMode = doc.groupId ? groupModeMap[doc.groupId] : doc.visibleType ? getVisibleGroupMode({
                type: doc.visibleType,
                offset: doc.offset,
                visibleGroups
              }) : mode;
          ...
          case "group":
            {
                ...
                case MODE_BREAK:
                  {
                    ...
                    const next = [ind, MODE_FLAT, doc.contents, doc.visibleType ? [...(visibleGroups || []), {
                      type: doc.visibleType,
                      mode: MODE_FLAT
                    }] : visibleGroups];
                    const rem = width - pos;
                        ...
                        if (doc.break) {
                          ...
                          for (let i = 1; i < doc.expandedStates.length + 1; i++) {
                            if (i >= doc.expandedStates.length) {
                              ...
                              const useMode = doc.firstBreakingIndex != null && i >= doc.firstBreakingIndex ? MODE_BREAK : MODE_FLAT;
                              const cmd = [ind, useMode, state, doc.visibleType ? [...(visibleGroups || []), {
                                type: doc.visibleType,
                                mode: useMode
                              }] : visibleGroups];
              ...
              if (!cmds[cmds.length - 1][3]) {
                cmds[cmds.length - 1][3] = doc.visibleType ? [...(visibleGroups || []), {
                  type: doc.visibleType,
                  mode: groupMode
                }] : visibleGroups;
              }
          ...
          case "fill":
            {
              ...
              const whitespaceBreakCmd = [ind, MODE_BREAK, whitespace, visibleGroups];
              ...
              parts.splice(0, 2);
              const remainingCmd = [ind, mode, fill$1(parts), visibleGroups];
              const secondContent = parts[0];
              const firstAndSecondContentFlatCmd = [ind, MODE_FLAT, concat$1([content, whitespace, secondContent]), visibleGroups];
