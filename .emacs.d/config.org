...
- [[#theme][Theme]]
- [[#ui][UI]]
  - [[#maximize-properly-on-macos][Maximize Properly on MacOS]]
...
#+begin_src emacs-lisp
  ...
  (add-hook 'text-mode-hook (lambda () (electric-pair-local-mode 0)))
#+end_src

See exception for org-mode: [[*Create Blocks In Org Files][Create Blocks In Org Files]]
...
* UI
** Maximize Properly on MacOS
Without this, when I maximize an Emacs frame, there is a gap between the bottom edge of the frame and the bottom edge of the computer display.
#+begin_src emacs-lisp
...
*** Embark: Take actions on search results
#+begin_src emacs-lisp
  (use-package embark
    :after general
    :ensure t
...
#+begin_src emacs-lisp
  ;; Orderless author's configuration as of 2025-04-04
...
**** kind-icon package to show icons next to auto-complete suggestions based on the type of suggestion (function name, variable name, etc)
#+begin_src emacs-lisp
...
**** Set up Cape to provide Completion At Point Extensions that somehow make Corfu better, especially with LSP auto-complete.
#+begin_src emacs-lisp
  (use-package cape
    :init
...
#       (cs/leader-keys
#         "SPC" '(helm-buffers-list :wk "Find buffer")
#         "b b" '(consult-buffer :wk "Find buffer"))

#       (cs/leader-keys
#         "f" '(:ignore t :wk "Find")
#         "f f" '(helm-find-files :wk "Find file")
#         "f g" '(helm-grep-do-git-grep :wk "Find file in Git")
#         "f r" '(helm-recentf :wk "Find recent file")
#         "f s" '(helm-do-grep-ag :wk "Find search term in project"))
#       )
# #+end_src
...
#+begin_src emacs-lisp
  (use-package lsp-mode
    :after (:all general orderless cape)
    :custom
    ;; https://github.com/minad/corfu/wiki#basic-example-configuration-with-orderless 2025-04-04
    (lsp-completion-provider :none) ;; we use Corfu!
    
    :init
    (defun cs/orderless-dispatch-flex-first (_pattern index _total)
      (and (eq index 0) 'orderless-flex))

    (defun cs/lsp-mode-setup-completion ()
      (setf (alist-get 'styles (alist-get 'lsp-capf completion-category-defaults))
            '(orderless))
      ;; Optionally configure the first word as flex filtered.
      (add-hook 'orderless-style-dispatchers #'cs/orderless-dispatch-flex-first nil 'local)
      ;; Optionally configure the cape-capf-buster.
      (setq-local completion-at-point-functions (list (cape-capf-buster #'lsp-completion-at-point))))

    :hook
    ((lsp-completion-mode . cs/lsp-mode-setup-completion)
     (clojure-mode . lsp)
    ...
    ;; Makes LSP shutdown the language server when all buffers in the project are closed.
...
** Treesitter
*** Install Grammars Manually
Many grammars can be built by cloning and running https://github.com/casouri/tree-sitter-module locally and then copying the generated grammars into `.emacs.d/tree-sitter`.

*** Configure Emacs treesit
#+begin_src emacs-lisp
